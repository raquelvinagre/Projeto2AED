...
Graph* GraphCopy(const Graph* g) {
  ...
  Graph* copy = GraphCreate(g->numVertices, g->isDigraph, g->isWeighted); // Create a new graph with the same properties as the original graph

  List* vertices = g->verticesList;
  ListMoveToHead(vertices); // Move to the first vertex
  unsigned int i = 0;

  for (; i < g->numVertices; ListMoveToNext(vertices), i++) {  
    struct _Vertex* v = ListGetCurrentItem(vertices); // Get the current vertex

    List* edges = v->edgesList; // Get the list of edges of the current vertex
    ...
    for (; j < ListGetSize(edges); ListMoveToNext(edges), j++) { 
      struct _Edge* e = ListGetCurrentItem(edges);
      if (g->isWeighted) {
        GraphAddWeightedEdge(copy, i, e->adjVertex, e->weight); // if the graph is weighted, add the edge with the weight
      } else {
        GraphAddEdge(copy, i, e->adjVertex); // if the graph is not weighted, add the edge without the weight
      }
    }
  }
...
Graph* GraphFromFile(FILE* f) {
  ...
  // Read the second line to determine if the graph is weighted or not
  int isWeighted;
  fscanf(f, "%d", &isWeighted);

  // Read the third line to get the number of vertices
  unsigned int numVertices;
  fscanf(f, "%u", &numVertices);

  // Read the fourth line to get the number of edges
  unsigned int numEdges;
  fscanf(f, "%u", &numEdges);

  // Create the graph based on the information obtained
  ...
  for (unsigned int i = 0; i < numEdges; ++i) {
    ...
    if (isWeighted) {
      double weight;
      fscanf(f, "%u %u %lf", &v, &w, &weight);
      // Check for a loop before adding the edge
      if (v != w) {
...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  // Remove the edge from vertexV's adjacency list
  List* edgesListV = vertexV->edgesList;
  ListMoveToHead(edgesListV);

  // Iterate through the edges of vertexV
  for (unsigned int i = 0; i < ListGetSize(edgesListV); ListMoveToNext(edgesListV), i++) {
    struct _Edge* edge = ListGetCurrentItem(edgesListV);
    // If the edge is found, remove it from the list and free the memory
    if (edge->adjVertex == w) {
      free(edge);
      ...
      vertexV->outDegree--;
      vertexW->inDegree--;
      // If it's an undirected graph, update the degree of vertexW
      if (!GraphIsDigraph(g)) {
        
        // Also remove the corresponding edge from vertexW's adjacency list
        List* edgesListW = vertexW->edgesList;
        ListMoveToHead(edgesListW);

        // Iterate through the edges of vertexW
        for (unsigned int j = 0; j < ListGetSize(edgesListW); ListMoveToNext(edgesListW), j++) {
          struct _Edge* reverseEdge = ListGetCurrentItem(edgesListW);
          if (reverseEdge->adjVertex == v) {
            free(reverseEdge);
            ListRemoveCurrent(edgesListW);
            vertexW->outDegree--;
            break;  // Found and removed the reverse edge
          }
        }
      }

      return 1;  // Successfully removed the edge
    }
  }
...
int GraphCheckInvariants(const Graph* g) { //
  ...
  for (; i < g->numVertices; ListMoveToNext(vertices), i++) {
    ...
    if (v->outDegree != ListGetSize(v->edgesList)) {
        return 0; // the outDegree of a vertex should be equal to the size of its edgesList (because it's a graph)
    }
  }
  return 1; // the graph is valid
}
